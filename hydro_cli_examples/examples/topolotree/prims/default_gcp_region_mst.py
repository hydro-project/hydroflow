...
class Graph():
    ...
    def __init__(self):
        self.gcp_regions = ["Iowa", "Oregon", "Northern Virginia", "South Carolina", "Belgium", "London", "Frankfurt", "Singapore", "Taiwan", "Tokyo",
    ...
    def create_graph(self):
        ...
        outer = []
        for i in range(len(self.gcp_regions)):
            inner = []
            for j in range(len(self.gcp_regions)):
                filter1 = df['From'] == self.gcp_regions[i]
                new_df = df.where(filter1).dropna()
                new_df = new_df[self.gcp_regions[j]]
    ...
    def printMST(self, parent):
        ...
        for lat, lon, name in zip(latitudes, longitudes, self.gcp_regions):
            folium.CircleMarker(
                location=[lat, lon],
                radius=3,
                color='red',
