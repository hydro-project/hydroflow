...
async fn main() {
    ...
    let df = hydroflow_syntax! {
        ...
        broadcaster = cross_join::<'static, 'tick>()
            -> map(|(peer, state): (_, Vec<(u64, bool, Rc<RefCell<(Vec<u32>, Vec<u32>)>>)>)| {
                (peer as u32, state.iter().filter(|t| t.1).map(|(k, _, v)| (*k, (my_id, v.as_ref().borrow().0[my_id], v.as_ref().borrow().1[my_id]))).collect())
            })
            -> filter(|(_, gossip): &(_, Vec<(u64, (usize, u32, u32))>)| !gossip.is_empty())
            -> map(|(peer, gossip): (_, _)| {
                (peer as u32, serialize_to_bytes(GossipOrIncrement::Gossip(gossip)))
            })
            -> dest_sink(to_peer);

        next_state
            -> flat_map(|a: Vec<(u64, bool, Rc<RefCell<(Vec<u32>, Vec<u32>)>>)>| {
