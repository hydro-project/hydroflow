...
async fn main() {
    ...
    let (my_id, quorum, p1a_timeout_const, i_am_leader_resend_timeout_const, i_am_leader_check_timeout_const):(u32, u32, u32, u32, u32) = 
...
iAmLeader(i, n) :+ iAmLeader(i, n), !iAmLeaderCheckTimeout() # clear iAmLeader periodically (like LRU clocks)
        ...
        p2aOut(a, i, no, slot, i, num) :- FilledHoles(no, slot), id(i), ballot(num), acceptors(a) # Weird bug where if this line is not commented out, id has an error?
        ...
        throughputOut(num) :- totalCommitted(num), p1aTimeout(), IsLeader() # acceptor commit rate
        // throughputOut(num) :- nextSlot(num), p1aTimeout(), IsLeader() # proposer input rate
        ...
        ResentLog(payload, slot) :- P1bLargestEntryBallot(slot, payloadBallotID, payloadBallotNum), P1bMatchingEntry(payload, slot, c, payloadBallotID, payloadBallotNum), !CommittedLog(otherPayload, slot), P1bQuorumReached(), !nextSlot(s)
        ...
        FilledHoles(no, s) :- noop(no), !ProposedSlots(s), PrevSlots(s), P1bQuorumReached(), !nextSlot(s2)
...
nextSlot(s+1) :+ P1bQuorumReached(), MaxProposedSlot(s), !nextSlot(s2)
