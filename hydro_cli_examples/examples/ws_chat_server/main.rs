...
async fn ws_server(
    ...
    tokio::spawn(async move {
        loop {
            if let Some((recipient, msg)) = to_client_recv.next().await {
...
async fn main() {
    ...
    let df = hydroflow_syntax! {
        ...
        messages_with_names = join::<'tick, 'tick>() -> tee();

        messages_with_names -> map(|(_, (msg, name))| format!("{}: {}", name, msg)) -> map(|s| Bytes::from(s)) -> dest_sink(to_logger);
    };
