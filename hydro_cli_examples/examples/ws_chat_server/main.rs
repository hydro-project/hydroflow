...
async fn main() {
    ...
    let (clients_connect, clients_disconnect, from_client, to_client) = util::ws_server(ws_port).await;
    ...
    let df = hydroflow_syntax! {
        ...
        all_messages = union() -> unique::<'static>();

        broadcast_clients = cross_join::<'tick, 'tick, HalfMultisetJoinState>() -> to_client;
        active_clients -> [0] broadcast_clients;
