...
async def main(args):
    ...
    for i in range(0, f+1):
        ...
        leader_retry_timeout = 1 if i == 0 else 10 # proposer with id 0 is much more likely to be the leader

        proposer = deployment.HydroflowCrate(
            src=".",
            example="dedalus_paxos_proposer",
            args=[json.dumps((i, 2*f+1, heartbeat_timeout, leader_retry_timeout))], # my_id, quorum, heartbeat_timeout_const, p1a_timeout_const
            on=machine1
        )
        ...
        for j in range(0, f+1):
            if i != j: # don't let proposers send to themselves
                proposer_i_am_leader_ports[i][j] = proposer.ports.i_am_leader_source.merge()
