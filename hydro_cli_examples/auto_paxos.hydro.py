...
async def main(args):
    ...
    p2b_proxy_leader_0_out = await p2b_proxy_leader_programs[0][0].stdout()
    leader_out = await proposer_programs[0].stdout()
    ...
    try:
        async with stream.merge(leader_out, p2b_proxy_leader_0_out).stream() as merged:
            async for log in merged:
                split = log.split(",")
                if split[0] == "throughput" and split[1] == "0":
                    throughput.append(int(split[2]) * num_p2b_proxy_leaders)
                # if split[0] == "tick":
