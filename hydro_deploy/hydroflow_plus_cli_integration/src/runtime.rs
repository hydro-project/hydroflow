...
use stageleft::internal::TokenStream;
...
impl<'a> Location<'a> for CLIRuntimeNode<'a> {
    ...
    fn flow_builder(&self) -> (&'a RefCell<usize>, &'a hydroflow_plus::builder::Builders, &'a RefCell<Vec<HfPlusNode>>) {
...
impl<'a> HfSendOneToOne<'a, CLIRuntimeNode<'a>> for CLIRuntimeNode<'a> {
    ...
    fn gen_sink_statement(&self, port: &String) -> TokenStream {
    ...
    fn gen_source_statement(other: &CLIRuntimeNode<'a>, port: &String) -> TokenStream {
...
impl<'a> HfSendOneToMany<'a, CLIRuntimeCluster<'a>> for CLIRuntimeNode<'a> {
    ...
    fn gen_source_statement(other: &CLIRuntimeCluster<'a>, port: &String) -> TokenStream {
        let self_cli = other.cli;
        let port = port.as_str();
