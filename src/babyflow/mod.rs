...
impl Dataflow {
    ...
    #[must_use]
    pub fn add_source<F, W, O>(
        &mut self,
        mut op_subtree: F,
    ) -> OutputPort<O, W::Writable>
    where
        F: 'static + FnMut(&SendCtx<O, W::Writable>),
        W: 'static + Handoff<O>,
        O: 'static,
    {
        self.add_op::<_, (), W, (), O>(move |_, send| op_subtree(send)).1
    }

    #[must_use]
    pub fn add_sink<F, R, I>(
        &mut self,
        mut op_subtree: F,
    ) -> InputPort<I, R::Writable>
    where
        F: 'static + FnMut(&RecvCtx<I, R::Readable>),
        R: 'static + Handoff<I>,
        I: 'static,
    {
        self.add_op::<_, R, (), I, ()>(move |recv, _| op_subtree(recv)).0
    }

    pub fn add_edge<H, T>(&mut self, output_port: OutputPort<T, H::Writable>, input_port: InputPort<T, H::Writable>)
    where
        H: Handoff<T>,
    {
