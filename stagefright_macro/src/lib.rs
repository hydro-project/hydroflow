...
pub fn q(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    proc_macro::TokenStream::from(quote!({
        ...
        move |set_mod: &mut String, set_str: &mut String, set_vec: &mut Vec<(String, (Option<#root::internal::TokenStream>, Option<#root::internal::TokenStream>))>, run: bool| {
...
pub fn entry(
    ...
    // TODO(shadaj): support entries in inner modules
    let input_name_str = input_name.to_string();
    let input_name_underscored =
        "crate".to_string() + "_" + "__flow" + "_" + input_name_str.as_str();
    let input_name_underscored_ident = syn::Ident::new(&input_name_underscored, Span::call_site());
    ...
    proc_macro::TokenStream::from(quote_spanned! {input.span()=>
        ...
        #orig_visibility fn #input_name(input: #root::internal::TokenStream) -> #root::internal::TokenStream {
            ...
            let module_path = module_path.segments.iter().skip(1).collect::<Vec<_>>();
            let module_path = #root::internal::syn::Path {
                leading_colon: None,
                segments: #root::internal::syn::punctuated::Punctuated::from_iter(module_path.into_iter().cloned()),
            };

            ::#root::internal::quote!({
                use #pound final_crate_path ::__flow:: #pound module_path *;
        ...
        #[cfg(feature = "final")]
        pub use crate::__macro::#input_name_underscored_ident as #input_name;
    })
}
