...
fn test() {
    ...
    let (_server, _, mut server_stdout) =
        run_cargo_example("kvs", "--role server --addr 127.0.0.100:2051");

    let (_client, mut client_stdin, mut client_stdout) = run_cargo_example(
        "kvs",
        "--role client --addr 127.0.0.101:2051 --server-addr 127.0.0.100:2051",
    );
    ...
    let (_client2, mut client2_stdin, mut client2_stdout) = run_cargo_example(
        "kvs",
        "--role client --addr 127.0.0.102:2051 --server-addr 127.0.0.100:2051",
    );
