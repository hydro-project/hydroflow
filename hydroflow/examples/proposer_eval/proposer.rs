...
pub(crate) async fn run_proposer(opts: Opts) {
    ...
    while total_counter < 300000 + 100 {
        ...
        total_flush_time += tick_time.as_secs() * 1000 + tick_time.subsec_nanos() as f64 / 1_000_000.0;

        counter += 1;
        total_counter += 1;
        if counter % 10000 == 0 {
            let elapsed = start.elapsed().unwrap();
            let elapsed_ms = elapsed.as_secs() * 1000 + elapsed.subsec_nanos() as f64 / 1_000_000.0;
            println!(
