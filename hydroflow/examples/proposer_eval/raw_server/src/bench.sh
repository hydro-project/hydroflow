...
do
    ./target/release/raw-proposer-eval --role acceptor --port $PORT --addr localhost --id 14 &
...
if [ "$MODE" = "control" ]; then
    ./target/release/raw-proposer-eval --role proposer --port 20000 --addr localhost --id 10 --acceptors $NUM_ACCEPTORS_USE --proxies $NUM_PROXY
elif [ "$MODE" = "proxy" ]; then

    for PORT in $(seq $PROXY_MIN_PORT $PROXY_MAX_PORT);
    do
        ./target/release/raw-proposer-eval --role proxy-leader  --port $PORT --addr localhost --id 14 --acceptors $NUM_ACCEPTORS_USE --proxies $NUM_PROXY &
        bgpids+=($!)
    done

    sleep 2

    ./target/release/raw-proposer-eval --role proposer --port 20000 --addr localhost --id 10 --use-proxy --acceptors $NUM_ACCEPTORS_USE --proxies $NUM_PROXY
    #pid=$!
