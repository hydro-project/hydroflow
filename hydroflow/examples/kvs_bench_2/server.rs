    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
...

            {
                ...
                for _i in 0..100 {
                    client_to_transducer_tx.send((KVSRequest2::Put {
                        key: dist.sample(&mut rng) as u64,
                        value: Bytes::from_static(&[0; 1024]),
                    }, Vec::new())).unwrap();
                }
            }
            ...
            let mut df = hydroflow_syntax! {
                ...
                client_input = source_stream(client_to_transducer_rx)
                    -> demux(|(req, addr): (KVSRequest2, Vec<u8>), var_args!(puts, gets)| {
                        match req {
                            KVSRequest2::Put {key, value} => puts.give((key, (ValueOrReg::Value(value), addr))),
                ...
                put_ack_tee
                    ...
                    -> map(|(key, (_reg, addr))| (KVSResponse2::PutResponse{key}, addr.unwrap()))
