...
impl<'a> HfCluster<'a> for CLIRuntimeCluster<'a> {
    fn ids(&self) -> impl Quoted<'a, &'a Vec<usize>> {
        let cli = self.cli;
        let self_id = self.id;
...
impl CLIRuntimeNodeBuilder<'_> {
    pub fn new(cli: RuntimeData<&HydroCLI<HydroflowPlusMeta>>) -> CLIRuntimeNodeBuilder {
...
pub struct CLIRuntimeClusterBuilder<'a> {
    cli: RuntimeData<&'a HydroCLI<HydroflowPlusMeta>>,
}

impl CLIRuntimeClusterBuilder<'_> {
    pub fn new(cli: RuntimeData<&HydroCLI<HydroflowPlusMeta>>) -> CLIRuntimeClusterBuilder {
        CLIRuntimeClusterBuilder { cli }
    }
}

impl<'cli> HfClusterBuilder<'cli, CLIRuntime> for CLIRuntimeClusterBuilder<'cli> {
    fn build(
        &mut self,
        id: usize,
        builder: &'cli HfBuilder<'cli, CLIRuntime>,
