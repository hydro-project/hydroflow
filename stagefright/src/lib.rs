...
pub use stagefright_macro::{entry, q, quse_fn};
...
impl<T> ToTokens for QuotedExpr<T> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let module_path = syn::Path {
            leading_colon: None,
            segments: syn::punctuated::Punctuated::from_iter(module_path.into_iter().cloned()),
        };

        let expr = &self.expr;
        tokens.extend(quote!({
            use ::#final_crate_path::__flow::#module_path *;
