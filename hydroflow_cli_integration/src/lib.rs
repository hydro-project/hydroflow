...
pub trait Connected: Send {
    ...
    async fn from_defn(pipe: ServerPort) -> Self;
}
...
impl Clone for BoundConnection {
    fn clone(&self) -> Self {
        panic!("Cannot clone a BoundConnection, but this should never happen")
    }
}
...
impl Connected for ConnectedBidi {
    ...
    async fn from_defn(pipe: ServerPort) -> Self {
        match pipe {
            ...
            ServerPort::Demux(_) => panic!("Cannot connect to a demux pipe directly"),
        }
    }
}
                ...
                if let BoundConnection::Demux(demux) = bound {
                    ...
                    for (id, bound) in demux {
                        connected_demux.insert(
                            id,
                            Arc::new(Mutex::new(
                                T::from_defn(ServerPort::Bind(bound)).await.take_sink(),
                            )),
                        );
                    }
