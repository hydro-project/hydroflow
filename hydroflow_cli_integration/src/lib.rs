...
impl From<&ServerPort> for RealizedServerPort {
    fn from(port: &ServerPort) -> Self {
        match port {
            ...
            ServerPort::Demux(bindings) => RealizedServerPort::Demux(
                bindings.into_iter().map(|(k, v)| (*k, v.into())).collect(),
            ),
            ServerPort::Merge(ports) => {
                RealizedServerPort::Merge(ports.into_iter().map(|p| p.into()).collect())
            }
