...
If you don't care which instance sent the data is to, you can use `send_{bincode,bytes}_interleaved`, where the recipient receives a stream of `T` elements, but the elements received from different instances will be interleaved.
```rust
let stream = cluster.source_iter(q!(vec![123, 456, 789]))
