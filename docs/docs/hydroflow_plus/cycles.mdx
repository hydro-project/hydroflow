...
We can create a cycle by using the `cycle` method on a process or cluster. This returns a tuple of two values: a `HfCycle` value that can be used to complete the cycle later and the placeholder stream.
