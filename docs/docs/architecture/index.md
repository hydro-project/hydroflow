...
[reachability](./example_5_reachability) chapter, which we will use a running example:
...
graphs that get parsed and compiled into a scheduled graph of one or more compiled subgraphs. Please see the [Surface Syntax](./surface_syntax) docs for more information.
