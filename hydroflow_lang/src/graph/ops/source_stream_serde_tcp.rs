...
///     let (outbound, inbound, _) = hydroflow::util::tcp_bytes(addr).await;
///     let mut flow = hydroflow::hydroflow_syntax! {
///         source_stream_serde_tcp(inbound) -> map(|(x, a): (String, std::net::SocketAddr)| x.to_uppercase())
...
pub const SOURCE_STREAM_SERDE_TCP: OperatorConstraints = OperatorConstraints {
    name: "source_stream_serde_tcp",
        ...
        let write_iterator = quote_spanned! {op_span=>
            let #ident = std::iter::from_fn(|| {
                match #root::futures::stream::Stream::poll_next(#stream_ident.as_mut(), &mut std::task::Context::from_waker(&context.waker())) {
                    std::task::Poll::Ready(Some(std::result::Result::Ok(payload))) => Some(#root::util::deserialize_from_bytes(payload)),
