...
///             -> map (|m| (m, remote)) -> dest_sink_serde_tcp(outbound);
///     };
///     flow.run_available();
/// }
/// ```
#[hydroflow_internalmacro::operator_docgen]
pub const DEST_SINK_SERDE_TCP: OperatorConstraints = OperatorConstraints {
    name: "dest_sink_serde_tcp",
            ...
            df
                .spawn_task(async move {
                    ...
                    while let Some(payload) = recv.recv().await {
                        let item = #root::util::serialize_to_bytes(payload);
                        ...
                        while let Ok(payload) = recv.try_recv() {
                            let item = #root::util::serialize_to_bytes(payload);
                            sink.feed(item).await.expect("Error processing async sink item.");
                        }
