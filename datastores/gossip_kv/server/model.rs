use hydroflow::lattices::{DomPair, Max, WithBot};
...
mod tests {
    ...
    use crate::model::{Clock, delete_row, upsert_row, TableMap, TableName, RowKey};
    ...
    fn test_table_set() {
        ...
        let third_tick: Clock = Clock::new(2);
        ...
        let insert = upsert_row(first_tick, members_table.clone(), key_1.clone(), value_1.clone());
        Merge::merge(&mut table_map, insert);
        {
            ...
            assert_eq!(*row.unwrap().as_reveal_ref().0, first_tick, "Unexpected row timestamp");

            let value = row.unwrap().as_reveal_ref().1.as_reveal_ref();
            assert_eq!(value, &HashSet::from([value_1.to_string()]), "Unexpected row value");
        }
        ...
        Merge::merge(&mut table_map, delete_row);
        {
            ...
            assert!(row.is_some(), "Row should exist");
            assert_eq!(*row.unwrap().as_reveal_ref().0, second_tick, "Unexpected row timestamp");
