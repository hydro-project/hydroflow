...
pub enum GossipRequestWithAddress<A> {
    ...
    Ack { message_id: String, member_id: String, addr: A },
    /// A nack request with the message id and the address of the client.
    Nack { message_id: String, member_id: String, addr: A },
}
...
impl<A> GossipRequestWithAddress<A> {
    ...
    pub fn from_request_and_address(request: GossipMessage, addr: A) -> Self {
        match request {
            ...
            GossipMessage::Ack { message_id, member_id } => Self::Ack { message_id, addr, member_id },
            GossipMessage::Nack { message_id, member_id } => Self::Nack { message_id, addr, member_id },
        }
    }
}
