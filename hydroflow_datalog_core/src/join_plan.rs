...
use crate::grammar::datalog::{
    BoolExpr, BoolOp, ExtractExpr, InputRelationExpr, IntExpr, RhsField,
};
...
pub enum JoinPlan<'a> {
    ...
    MagicNatLt(Box<JoinPlan<'a>>, RhsField, RhsField),
}
...
fn process_extract(
    ...
    match extract {
        ...
        ExtractExpr::Flatten(_, expr) => {
            let inner = process_extract(expr, i, variable_mapping, local_constraints, to_flat);
            to_flat.push(inner);
            inner
        }
    }
    ...
    match plan {
        JoinPlan::Source(target, persisted) => {
            ...
            for (i, ident) in target.fields.iter().enumerate() {
                ...
                if let RhsField::Extract(extract) = ident.deref() {
                    process_extract(
                        extract,
                        i,
            ...
            for index in indices_to_flat {
                let tuple_elems_post_flat = (0..row_types.len())
                    .map(|i| {
                        if i == index {
                ...
                let flat_idx = syn::Index::from(index);

                pipeline = parse_quote_spanned! {get_span(rule_span)=>
                    #pipeline -> flat_map(|row: #row_type| row.#flat_idx.into_iter().map(move |__flattened_element| (#(#tuple_elems_post_flat, )*)))
                };
            }
        ...
        JoinPlan::MagicNatLt(inner, less_than, threshold) => {
            ...
            if let RhsField::Extract(extract) = less_than {
                match extract {
                    ExtractExpr::Ident(ident) => {
            ...
            let threshold_name = if let RhsField::Extract(ExtractExpr::Ident(threshold)) = threshold
            {
            ...
            if let RhsField::Extract(ExtractExpr::Ident(less_than)) = less_than {
