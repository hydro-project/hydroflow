...
use syn::visit::Visit;
use syn::{
    parse_macro_input, AngleBracketedGenericArguments, GenericParam, MacroDelimiter, PathArguments,
    Token, Type,
};
...
pub fn q(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let free_variables = visitor.free_variables.iter().map(|i| {
        let ident = i.clone();
        let ident_str = ident.to_string();
        quote!((#ident_str.to_string(), ::#root::quoting::ToFreeVariableTokens::to_tokens(&#ident)))
    });
    ...
    let unitialized_free_variables = visitor.free_variables.iter().map(|i| {
        ...
        quote!(
            ...
            let #i = ::#root::quoting::FreeVariable::uninitialized(#i_without_span)
        )
    });
            ...
            if is_rooted {
                ...
                if global {
                    into.push(quote! {
                        ...
                        #[allow(non_upper_case_globals, non_snake_case)]
                        fn #name_ident() -> #root::quoting::Import<(), u32> {
                            #root::quoting::create_import(
                                #full_path,
                                {
                                    let __quse_local = ();
                                    {
                    ...
                    into.push(quote! {
                        use ::#(#prefix_unspanned::)*#name_ident_unspanned;
                        #[allow(non_upper_case_globals, non_snake_case)]
                        let #name_ident = #root::quoting::create_import(
                            #full_path,
                            {
                                let __quse_local = ();
                                {
                ...
                into.push(quote! {
                    #[allow(non_upper_case_globals, non_snake_case)]
                    let #name_ident = #root::quoting::Import::clone(&#name_ident);
                });
            }
        }
        _ => todo!(),
...
#[proc_macro]
pub fn quse_type(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let hydroflow_plus_crate = proc_macro_crate::crate_name("hydroflow_plus")
        .expect("hydroflow_plus should be present in `Cargo.toml`");
    let root = match hydroflow_plus_crate {
        proc_macro_crate::FoundCrate::Itself => quote! { hydroflow_plus },
...
pub fn qtype(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    quote! {
        ...
        #[allow(non_upper_case_globals, non_snake_case)]
        fn #name() -> #root::quoting::Type {
            #root::quoting::Type::new(#definition_string)
        }
    }
    .into()
}
...
pub fn flow(
            ...
            syn::FnArg::Typed(pat_type) => {
                ...
                if let Some(runtime_tpe) = runtime_tpe {
                    ...
                    visitor.free_variables.iter().for_each(|i| {
                        out.push(quote! {
                            if let Some(prelude) = ::#root::quoting::ToFreeVariableTokens::to_tokens(&#i).0 {
                    ...
                    vec![quote_spanned! {input.span()=>
                        let #pat: #ty = #root::quoting::ParseFromLiteral::parse_from_literal(&input_parsed[#i]);
                    }]
                }
            }
        }
    ...
    let params_to_pass = input.sig.inputs.iter().skip(1).map(|input| match input {
        ...
        syn::FnArg::Typed(pat_type) => {
            ...
            if is_runtime {
                ...
                quote!(#root::quoting::RuntimeData::new(#pat_str))
            } else {
                let pat = pat_type.pat.clone();
                quote!(#pat)
            }
    ...
    let first_generic = input_generics
        .params
        .iter()
        .find(|g| matches!(g, GenericParam::Lifetime(_)))
        .cloned();

    let hf_generics = first_generic.map(|g| quote!(<#g>));
    ...
    proc_macro::TokenStream::from(quote_spanned! {input.span()=>
        ...
        pub fn #input_name(input: ::proc_macro::TokenStream) -> ::proc_macro::TokenStream {
            ...
            let dataflow_core = #root::hydroflow_build(|| {
                #input_name #passed_generics(&#root::HfGraph::new(), #(#params_to_pass),*);
            });
            ...
            ::proc_macro::TokenStream::from(::#root::internal::quote!({
                ...
                fn create_flow #input_generics(
                    #(#runtime_data_params),*
                ) -> #root::hydroflow::scheduled::graph::Hydroflow #hf_generics {
                    #pound dataflow_core
                }
