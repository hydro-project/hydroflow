...
pub fn entry(
            ...
            syn::FnArg::Typed(pat_type) => {
                ...
                if let Some(runtime_tpe) = runtime_tpe {
                    ...
                    visitor.free_variables.iter().for_each(|i| {
                        ...
                        out.push(quote! {
                            if let Some(prelude) = ::#root::quoting::ToFreeVariableTokens::to_tokens(&#i_unspanned).0 {
    ...
    proc_macro::TokenStream::from(quote_spanned! {input.span()=>
        ...
        pub fn #input_name(input: ::proc_macro::TokenStream) -> ::proc_macro::TokenStream {
            ...
            #input
            let dataflow_core = {
                let graph = #root::quoting::QuotedContext::create();
                #root::quoting::Quoted::build(#input_name #passed_generics(&graph, #(#params_to_pass),*))
            };
