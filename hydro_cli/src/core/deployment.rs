...
impl Deployment {
    pub async fn deploy(&mut self) -> Result<()> {
        ...
        let active_services = self.services.iter().filter(|service| service.upgrade().is_some()).cloned().collect::<Vec<_>>();
        self.services = active_services;

        for service in self.services.iter_mut() {
            service.upgrade().unwrap().write().await.collect_resources(&mut resource_batch);
        }
        ...
        let result = Arc::new(resource_batch.provision(&mut self.resource_pool, self.last_resource_result.clone()).await?);
        ...
        let services_future =
            self.services
                ...
                .map(|service: &mut Weak<RwLock<dyn Service>>| async {
                    service.upgrade().unwrap().write().await.deploy(&result).await;
                });
