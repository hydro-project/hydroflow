use std::{
    ...
    sync::{Arc, RwLock}, io::{BufReader, BufRead},
};
    ...
    for line in lines {
        if let Ok(line) = line {
            ...
            if let Some(first) = split.next() {
                if first.chars().all(|c| c != ' ') && split.next().is_some() && split.next().is_none() {
...
fn destroy_deployment(deployment_folder: &TempDir) {
    ...
    let mut destroy_child = Command::new("terraform")
