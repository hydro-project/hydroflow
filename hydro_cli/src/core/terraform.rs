use std::{collections::HashMap, sync::{Arc, RwLock}, process::Child};
...
/// Survives across multiple deployments, handles cleanup.
...
impl TerraformPool {
    pub fn new() -> TerraformPool {
        TerraformPool {
            counter: 0,
            active_applies: HashMap::new(),
        }
    }

    fn create_apply(&mut self, deployment_folder: TempDir) -> Result<(u32, Arc<tokio::sync::RwLock<TerraformApply>>)> {
        let next_counter = self.counter;
        self.counter += 1;

        let mut apply_command = std::process::Command::new("terraform");
        
        apply_command.current_dir(deployment_folder.path())
            .arg("apply")
            .arg("-auto-approve");
...
impl Drop for TerraformApply {
    fn drop(&mut self) {
        ...
        // #[cfg(unix)]
        // nix::sys::signal::kill(Pid::from_raw(self.child_pid as i32), nix::sys::signal::Signal::SIGINT).unwrap();
