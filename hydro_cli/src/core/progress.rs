...
impl BarTree {
    ...
    fn refresh_message(&mut self, cur_path: &[String], multi: &mut MultiProgress) {
        match self {
            BarTree::Root(children) => {
                for child in children {
                    child.refresh_message(cur_path, multi);
                }
            }
            BarTree::Group(name, pb, children) => {
                ...
                for child in children {
                    child.refresh_message(&path_with_group, multi);
                }
            }
...
impl ProgressTracker {
    ...
    pub fn start_task(
        ...
        let previous_bar = surrounding_children
            ...
            .flat_map(|c| c.get_pb())
            .next();
        let created_bar = if previous_bar.is_none() {
            if let Some(group_pb) = surrounding_pb {
            ...
            self.multi_progress
                .insert_after(previous_bar.unwrap(), core_bar)
        };
        ...
        self.tree.refresh_message(&[], &mut self.multi_progress);
        (inserted_index, pb)
    }
    ...
    pub fn leaf_with_progress<'a, T, F: Future<Output = T>>(
        ...
        let (leaf_i, bar) = {
            let mut progress_bar = PROGRESS_TRACKER
                ...
                .lock()
                .unwrap();
            progress_bar.start_task(group.clone(), name.to_string(), false, true)
        };
