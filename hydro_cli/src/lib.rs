...
use hydroflow_cli_integration::{
    ConnectedBidi, ConnectedSink, ConnectedSource, DynSink, DynStream, ServerOrBound, ServerPort,
};
...
#[pyclass]
struct PythonServerPort {
    underlying: ServerPort,
    underlying_realized: Option<ServerOrBound>,
}

#[pymethods]
impl PythonServerPort {
...
"#,
    ...
    *TOKIO_RUNTIME.write().unwrap() = Some(tokio::runtime::Runtime::new().unwrap());
    ...
    module.add_class::<PythonServerPort>()?;
