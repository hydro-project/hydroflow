    ...
    fn new() -> Self {
        Deployment {
            underlying: Arc::new(RwLock::new(crate::core::Deployment {
                hosts: Vec::new(),
                services: Vec::new(),
                resource_pool: Arc::new(RwLock::new(crate::core::ResourcePool::new())),
            })),
        }
    }
        ...
        Ok(Py::new(
            ...
            PyClassInitializer::from(Host {
                underlying: self.underlying.blocking_write().add_host(|id| {
                    crate::core::GCPComputeEngineHost::new(id, project, machine_type, image, region, network.underlying)
                }),
            })
            .add_subclass(LocalhostHost {}),
        )?
