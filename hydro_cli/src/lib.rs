...
use hydroflow_cli_integration::{Connected, ConnectedBidi, DynSink, DynStream};
...
// trait TakableSink: Send + Sync {
//     fn take_sink(&mut self) -> Arc<dyn crate::core::hydroflow_crate::ports::HydroflowSink>;
// }

// impl<T: crate::core::hydroflow_crate::ports::HydroflowSink + 'static> TakableSink for Option<T> {
//     fn take_sink(&mut self) -> Box<dyn crate::core::hydroflow_crate::ports::HydroflowSink> {
//         Box::new(self.take().unwrap())
//     }
// }
...
impl CustomClientPort {
    fn send_to(&mut self, to: &HydroflowSink) {
        self.underlying
            .try_write()
            .unwrap()
            .send_to(to.underlying.try_read().unwrap().deref());
    }
...
pub fn _core(py: Python<'_>, module: &PyModule) -> PyResult<()> {
    ...
    module.add_class::<ConnectedStream>()?;
