...
pub fn gen_macro(staged_path: &Path, final_crate: &str) {
...
impl VisitMut for GenFinalPubVistor {
    ...
    fn visit_file_mut(&mut self, i: &mut syn::File) {
        i.items.retain(|i| match i {
            syn::Item::Macro(m) => {
                let final_attr: syn::Attribute = parse_quote!(#[cfg(not(feature = "macro"))]);
...
pub fn gen_final(final_crate: &str) {
    let out_dir = env::var_os("OUT_DIR").unwrap();
