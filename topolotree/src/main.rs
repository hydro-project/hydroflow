...
async fn main() {
    ...
    let neighbors: Vec<u32> = args.into_iter().skip(1).map(|x| x.parse().unwrap()).collect();
    ...
    let flow = run_topolotree(self_id, neighbors, input_recv, operations_send, chan_tx, query_tx);
