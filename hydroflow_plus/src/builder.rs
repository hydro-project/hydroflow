...
pub struct HfBuilder<'a> {
    ...
    pub(crate) builders: RefCell<Option<HashMap<usize, FlatGraphBuilder>>>,
...
impl<'a> HfBuilder<'a> {
    ...
    pub fn build(&self, id: impl Quoted<'a, usize>) -> HfBuilt<'a> {
        let mut builders = self.builders.borrow_mut().take().unwrap();

        let mut conditioned_tokens = None;
        for (node_id, builder) in builders.drain() {
