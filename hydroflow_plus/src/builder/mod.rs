...
/// outputs of the dataflow. And the final hashmap is used to generate
/// unique identifiers for cycles.
pub type FlowState = Rc<RefCell<(Option<Vec<HfPlusLeaf>>, usize, HashMap<usize, usize>)>>;
...
impl<'a> FlowBuilder<'a> {
    ...
    pub fn new() -> FlowBuilder<'a> {
        FlowBuilder {
            flow_state: Rc::new(RefCell::new((Some(Vec::new()), 0, HashMap::new()))),
    ...
    pub fn finalize(mut self) -> built::BuiltFlow<'a> {
        ...
        built::BuiltFlow {
            ir: self.flow_state.borrow_mut().0.take().unwrap(),
