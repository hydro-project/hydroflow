...
impl<
        T,
        F: Fn(&mut String, &mut FreeVariables) -> T,
    > IntoQuoted<T> for F
{
}
...
impl<T> HydroflowNode<T> {
    ...
    pub fn filter<F: Fn(&T) -> bool>(&self, f: impl IntoQuoted<F>) -> HydroflowNode<T> {
        let next_id = HYDROFLOW_NEXT_ID.with(|next_id| {
            let mut next_id = next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
