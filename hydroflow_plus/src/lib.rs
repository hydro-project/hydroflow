...
pub trait FreeVariable<O>: Clone {
...
impl<T> Import<T> {
    ...
    pub fn extend(&self, path: &str, _unused_type_check: T) -> Import<T> {
        Import {
...
pub trait IntoQuoted<'a, T>: FnMut(&mut String, &mut FreeVariables, bool) -> T + 'a
where
    Self: Sized,
{
...
impl<'a, T, F: FnMut(&mut String, &mut FreeVariables, bool) -> T + 'a> IntoQuoted<'a, T> for F {}
...
impl<'a, T> HydroflowNode<'a, T> {
    ...
    pub fn map<U, F: Fn(T) -> U + 'a>(&self, f: impl IntoQuoted<'a, F>) -> HydroflowNode<'a, U> {
        let next_id = HYDROFLOW_NEXT_ID.with(|next_id| {
            let mut next_id = next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
    ...
    pub fn for_each<F: Fn(T) + 'a>(&self, f: impl IntoQuoted<'a, F>) {
        let next_id = HYDROFLOW_NEXT_ID.with(|next_id| {
            let mut next_id = next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
