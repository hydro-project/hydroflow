...
impl<'a, T, N: Location<'a>> CycleComplete<'a, Tick> for Stream<T, Bounded, Tick, N> {
    fn complete(self, ident: syn::Ident) {
        ...
        me.flow_state.borrow_mut().0.as_mut().expect("Attempted to add a leaf to a flow that has already been finalized. No leaves can be added after the flow has been compiled.").push(HfPlusLeaf::CycleSink {
            ident,
...
impl<'a, T, W, N: Location<'a>> CycleComplete<'a, NoTick> for Stream<T, W, NoTick, N> {
    fn complete(self, ident: syn::Ident) {
        self.flow_state.borrow_mut().0.as_mut().expect("Attempted to add a leaf to a flow that has already been finalized. No leaves can be added after the flow has been compiled.").push(HfPlusLeaf::CycleSink {
            ident,
...
impl<'a, T, W, C, N: Location<'a>> Stream<T, W, C, N> {
    ...
    pub fn dest_sink<S: Unpin + Sink<T> + 'a>(self, sink: impl Quoted<'a, S>) {
        self.flow_state.borrow_mut().0.as_mut().expect("Attempted to add a leaf to a flow that has already been finalized. No leaves can be added after the flow has been compiled.").push(HfPlusLeaf::DestSink {
...
impl<'a, T, W, N: Location<'a>> Stream<T, W, NoTick, N> {
    pub fn for_each<F: Fn(T) + 'a>(self, f: impl IntoQuotedMut<'a, F>) {
        self.flow_state.borrow_mut().0.as_mut().expect("Attempted to add a leaf to a flow that has already been finalized. No leaves can be added after the flow has been compiled.").push(HfPlusLeaf::ForEach {
            input: Box::new(HfPlusNode::Unpersist(Box::new(self.ir_node.into_inner()))),
            f: f.splice_fn1().into(),
        });
    }
...
impl<'a, T, W, N: Location<'a>> Stream<T, W, NoTick, N> {
        ...
        // for now, we restirct Out<CoreType> to be CoreType, which means no tagged cluster -> external
    {
        ...
        let mut leaves_borrow = self.flow_state.borrow_mut();

        let external_key = leaves_borrow.1;
        leaves_borrow.1 += 1;

        let leaves = leaves_borrow.0.as_mut().expect("Attempted to add a leaf to a flow that has already been finalized. No leaves can be added after the flow has been compiled()");
