...
use crate::node::{HfDeploy, HfNode, HfSendTo};
use crate::HfBuilder;

pub struct HfStream<'a, T, D: HfDeploy<'a> + ?Sized, N: HfNode<'a, D>> {
...
impl<'a, T, D: HfDeploy<'a>, N: HfNode<'a, D>> HfStream<'a, T, D, N> {
    ...
    pub fn inspect<F: Fn(&T) + 'a>(&self, f: impl IntoQuotedMut<'a, F>) -> HfStream<'a, T, D, N> {
        let next_id = {
            let mut next_id = self.graph.next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
    ...
    pub fn persist(&self) -> HfStream<'a, T, D, N> {
        let next_id = {
            let mut next_id = self.graph.next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
    ...
    pub fn delta(&self) -> HfStream<'a, T, D, N> {
        let next_id = {
            let mut next_id = self.graph.next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
    ...
    pub fn unique(&self) -> HfStream<'a, T, D, N>
    where
        T: Eq + Hash,
    {
        let next_id = {
    ...
    pub fn cross_product<O>(&self, other: &HfStream<'a, O, D, N>) -> HfStream<'a, (T, O), D, N> {
        let next_id = {
            let mut next_id = self.graph.next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
    ...
    pub fn union(&self, other: &HfStream<'a, T, D, N>) -> HfStream<'a, T, D, N> {
        let next_id = {
            let mut next_id = self.graph.next_id.borrow_mut();
            let id = *next_id;
            *next_id += 1;
...
impl<'a, D: HfDeploy<'a>, N: HfNode<'a, D>> HfStream<'a, Bytes, D, N> {
    pub fn send_to<N2: HfNode<'a, D>>(
        &self,
        other: &N2,
    ) -> HfStream<'a, Result<BytesMut, io::Error>, D, N2>
    where
        N: HfSendTo<'a, D, N2>,
    {
        let self_ident = &self.ident;
...
impl<'a, K, V1, D: HfDeploy<'a>, N: HfNode<'a, D>> HfStream<'a, (K, V1), D, N> {
    pub fn join<V2>(&self, n: &HfStream<'a, (K, V2), D, N>) -> HfStream<'a, (K, (V1, V2)), D, N>
    where
        K: Eq + Hash,
    {
        let next_id = {
