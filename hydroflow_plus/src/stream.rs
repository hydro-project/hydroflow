...
impl<'a, T, W, N: Location<'a>> Stream<'a, T, W, N> {
    fn pipeline_op<U, W2>(
        ...
        Stream {
            ...
            is_delta: produces_delta(self.is_delta),
            _phantom: PhantomData,
        }
    }
    ...
    pub(crate) fn ensure_concrete(self) -> Stream<'a, T, Windowed, N> {
        if self.is_delta {
            self.pipeline_op(parse_quote!(persist()), |_| false)
        } else {
    ...
    pub fn map<U, F: Fn(T) -> U + 'a>(self, f: impl IntoQuotedMut<'a, F>) -> Stream<'a, U, W, N> {
        let f = f.splice();
        self.pipeline_op(parse_quote!(map(#f)), |d| d)
    }
    ...
    pub fn filter<F: Fn(&T) -> bool + 'a>(
        ...
        let f = f.splice();
        self.pipeline_op(parse_quote!(filter(#f)), |d| d)
    }
    ...
    pub fn filter_map<U, F: Fn(T) -> Option<U> + 'a>(
        ...
        let f = f.splice();
        self.pipeline_op(parse_quote!(filter_map(#f)), |d| d)
    }
...
impl<'a, T, N: Location<'a>> Stream<'a, T, Windowed, N> {
    pub fn fold<A, I: Fn() -> A + 'a, C: Fn(&mut A, T)>(
        ...
        if self.is_delta {
            self.pipeline_op(parse_quote!(fold::<'static>(#init, #comb)), |_| false)
        } else {
            self.pipeline_op(parse_quote!(fold::<'tick>(#init, #comb)), |_| false)
        }
    }
    ...
    pub fn reduce<C: Fn(&mut T, T) + 'a>(
        ...
        if self.is_delta {
            self.pipeline_op(parse_quote!(reduce::<'static>(#comb)), |_| false)
        } else {
            self.pipeline_op(parse_quote!(reduce::<'tick>(#comb)), |_| false)
        }
    }
    ...
    pub fn delta(self) -> Stream<'a, T, Windowed, N> {
        if self.is_delta {
            ...
            self.pipeline_op(parse_quote!(multiset_delta()), |_| false)
        }
    }
    ...
    {
        self.ensure_concrete()
            .pipeline_op(parse_quote!(unique::<'tick>()), |_| false)
    }
...
impl<'a, T: Clone, W, N: Location<'a>> Stream<'a, &T, W, N> {
    pub fn cloned(self) -> Stream<'a, T, W, N> {
        self.pipeline_op(parse_quote!(map(|d| d.clone())), |d| d)
    }
}
...
impl<'a, K: Eq + Hash, V, N: Location<'a>> Stream<'a, (K, V), Windowed, N> {
    pub fn fold_keyed<A, I: Fn() -> A + 'a, C: Fn(&mut A, V) + 'a>(
        ...
        if self.is_delta {
            self.pipeline_op(parse_quote!(fold_keyed::<'static>(#init, #comb)), |_| false)
        } else {
            self.pipeline_op(parse_quote!(fold_keyed::<'tick>(#init, #comb)), |_| false)
        }
    }
    ...
    pub fn reduce_keyed<F: Fn(&mut V, V) + 'a>(
        ...
        if self.is_delta {
            self.pipeline_op(parse_quote!(reduce_keyed::<'static>(#comb)), |_| false)
        } else {
            self.pipeline_op(parse_quote!(reduce_keyed::<'tick>(#comb)), |_| false)
        }
    }
}
