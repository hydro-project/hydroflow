...
impl<'a, T, L: Location<'a>, B, Order> Stream<T, L, B, Order> {
    ...
    pub fn filter<F: Fn(&T) -> bool + 'a>(
        ...
        let f = f.splice_fn1_borrow_ctx(&self.location.root()).into();
        Stream::new(
            self.location,
            HfPlusNode::Filter {
                f,
    ...
    pub fn cross_product<O>(self, other: Stream<O, L, B>) -> Stream<(T, O), L, B, Order>
    where
        T: Clone,
        O: Clone,
    {
    ...
    pub fn inspect<F: Fn(&T) + 'a>(self, f: impl IntoQuotedMut<'a, F, L::Root>) -> Stream<T, L, B> {
        let f = f.splice_fn1_borrow_ctx(&self.location.root()).into();
    ...
    Order: MinOrder<NoOrder, Min = NoOrder>,
{
    pub fn fold_commutative<A, I: Fn() -> A + 'a, F: Fn(&mut A, T)>(
        self,
        init: impl IntoQuotedMut<'a, I, L::Root>,
        comb: impl IntoQuotedMut<'a, F, L::Root>,
    ) -> Singleton<A, L, B> {
        let init = init.splice_fn0_ctx(&self.location.root()).into();
        let comb = comb.splice_fn2_borrow_mut_ctx(&self.location.root()).into();
    ...
    pub fn reduce_commutative<F: Fn(&mut T, T) + 'a>(
        ...
        let f = comb.splice_fn2_borrow_mut_ctx(&self.location.root()).into();
...
impl<'a, T, L: Location<'a> + NoTick, B, Order> Stream<T, L, B, Order> {
    ...
    pub fn for_each<F: Fn(T) + 'a>(self, f: impl IntoQuotedMut<'a, F, L::Root>) {
        let f = f.splice_fn1_ctx(&self.location.root()).into();
        self.location
            .flow_state()
            .borrow_mut()
    ...
    pub fn dest_sink<S: Unpin + futures::Sink<T> + 'a>(
        self,
        sink: impl QuotedWithContext<'a, S, L::Root>,
    ) {
        self.location
            .flow_state()
            .borrow_mut()
            ...
            .push(HfPlusLeaf::DestSink {
                sink: sink.splice_typed_ctx(&self.location.root()).into(),
                input: Box::new(self.ir_node.into_inner()),
            });
    }
}
...
impl<'a, T, C1, B, Order> Stream<T, Cluster<'a, C1>, B, Order> {
    ...
    {
        let self_node_id = ClusterSelfId {};

        self.map(q!(move |b| (
            ClusterId::from_raw(self_node_id.raw_id),
            b.clone()
        )))
