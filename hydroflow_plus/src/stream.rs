...
impl<'a, T, W, C, N: Location> Stream<'a, T, W, C, N> {
    ...
    pub fn enumerate(self) -> Stream<'a, (usize, T), W, C, N> {
        Stream::new(
            self.location_kind,
            self.ir_leaves,
    ...
    pub fn inspect<F: Fn(&T) + 'a>(
        ...
        Stream::new(
            ...
            HfPlusNode::Persist(Box::new(HfPlusNode::Inspect {
                f: f.splice().into(),
                input: Box::new(HfPlusNode::Delta(Box::new(self.ir_node.into_inner()))),
            })),
        )
    }
}
...
impl<'a, T, N: Location> Stream<'a, T, Unbounded, NoTick, N> {
    pub fn sample_every(
        ...
        self.tick_batch().continue_if(samples).all_ticks()
    }
}
...
impl<'a, K: Eq + Hash, V, C, N: Location> Stream<'a, (K, V), Bounded, C, N> {
    pub fn fold_keyed<A, I: Fn() -> A + 'a, F: Fn(&mut A, V) + 'a>(
        self,
        init: impl IntoQuotedMut<'a, I>,
        comb: impl IntoQuotedMut<'a, F>,
    ) -> Stream<'a, (K, A), Bounded, C, N> {
        Stream::new(
            self.location_kind,
            self.ir_leaves,
