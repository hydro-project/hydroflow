...
impl PropertyDatabase {
    pub fn add_commutative_tag<'a, I, A, F: Fn(&mut A, I), Q: Quoted<'a, F> + Clone>(&mut self, expr: Q) -> Q {
...
fn properties_optimize_node(node: HfPlusNode, db: &PropertyDatabase, seen_tees: &mut SeenTees) -> HfPlusNode {
    match node.transform_children(|node, seen_tees| properties_optimize_node(node, db, seen_tees), seen_tees) {
        HfPlusNode::ReduceKeyed {
            f,
            input,
        } if db.is_tagged_commutative(&f.0) => {
...
pub fn properties_optimize(ir: Vec<HfPlusLeaf>, db: &PropertyDatabase) -> Vec<HfPlusLeaf> {
    ...
    ir.into_iter()
        .map(|l| l.transform_children(|node, seen_tees| properties_optimize_node(node, db, seen_tees), &mut seen_tees))
        .collect()
}
...
mod tests {
    ...
    fn test_property_database() {
        ...
        assert!(!db.is_tagged_commutative(
            &(q!(|a: &mut i32, b: i32| *a += b).splice())
        ));

        db.add_commutative_tag(
            q!(|a: &mut i32, b: i32| *a += b)
        );
