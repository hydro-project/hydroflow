...
pub trait HFNodeBuilder<'a, N> {
    fn build(&mut self, graph: &'a HfBuilder<'a>) -> N;
}
...
impl<'a> HFNodeBuilder<'a, CLIRuntimeNode<'a>> for CLIRuntimeNodeBuilder<'a> {
    fn build(&mut self, builder: &'a HfBuilder<'a>) -> CLIRuntimeNode<'a> {
        CLIRuntimeNode::new(builder.next_node_id(), self.cli)
    }
}

pub trait HFDeploy<'a> {
    type Node: HfNode<'a> + HfConnectable<'a, Self::Node>;
    type NodeBuilder: HFNodeBuilder<'a, Self::Node>;
}
