...
pub trait Location<'a>: Clone {
    ...
    fn source_stream<T, E: FuturesStream<Item = T> + Unpin>(
        &self,
        e: impl QuotedWithContext<'a, E, Self::Root>,
    ) -> Stream<T, Self, Unbounded>
    where
        Self: Sized + NoTick,
    {
        let e = e.splice_untyped_ctx(&self.root());
    ...
    fn singleton<T: Clone>(
        &self,
        e: impl QuotedWithContext<'a, T, Self::Root>,
    ...
    fn source_interval(
        &self,
        interval: impl QuotedWithContext<'a, Duration, Self::Root> + Copy + 'a,
    ...
    fn source_interval_delayed(
        &self,
        delay: impl QuotedWithContext<'a, Duration, Self::Root> + Copy + 'a,
        interval: impl QuotedWithContext<'a, Duration, Self::Root> + Copy + 'a,
    ) -> Stream<tokio::time::Instant, Self, Unbounded>
    where
        Self: Sized + NoTick,
