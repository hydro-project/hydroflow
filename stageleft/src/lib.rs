...
pub trait Quoted<T>: Sized + FreeVariable<T> {
...
pub trait IntoQuotedOnce<'a, T>:
    FnOnce(&mut String, &mut &'static str, &mut TokenStream, &mut FreeVariables, bool) -> T + 'a
{
...
impl<
        'a,
        T,
        F: FnOnce(&mut String, &mut &'static str, &mut TokenStream, &mut FreeVariables, bool) -> T
            + 'a,
    > IntoQuotedOnce<'a, T> for F
{
}

impl<'a, T> Quoted<T> for Box<dyn IntoQuotedOnce<'a, T>> {}

impl<'a, T> FreeVariable<T> for Box<dyn IntoQuotedOnce<'a, T>> {
...
pub trait IntoQuotedMut<'a, T>:
    FnMut(&mut String, &mut &'static str, &mut TokenStream, &mut FreeVariables, bool) -> T + 'a
{
}
...
impl<
        'a,
        T,
        F: FnMut(&mut String, &mut &'static str, &mut TokenStream, &mut FreeVariables, bool) -> T + 'a,
