        ...
        |c_to_proposers| {
            let to_proposers = unsafe {
                ...
                c_to_proposers
                    ...
                    .cross_singleton(cur_leader_id.latest_tick(&client_tick))
                    .all_ticks()
            }
            ...
            let (c_quorum_payloads, _) = collect_quorum::<_, _, _, _, ()>(
                &client_tick,
                unsafe {
                    // SAFETY: we immediately collect `all_ticks` after identifying quorums, and batches
                    // here only affect the batching of the outputs, not the elements, so we are safe
