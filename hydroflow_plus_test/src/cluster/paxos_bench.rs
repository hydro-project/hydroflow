...
use super::paxos::{Acceptor, PaxosPayload, Proposer};
...
pub fn paxos_bench(
    ...
    let (c_to_proposers_complete_cycle, c_to_proposers) =
        flow.cycle::<Stream<(u32, ClientPayload), Unbounded, NoTick, Cluster<Client>>>(&clients);
...
fn paxos_with_replica<'a>(
    ...
    Cluster<Proposer>,
    Cluster<Acceptor>,
    Stream<'a, super::paxos::Ballot, Unbounded, NoTick, Cluster<Proposer>>,
    ...
    let (r_to_acceptors_checkpoint_complete_cycle, r_to_acceptors_checkpoint) =
        flow.cycle::<Stream<i32, Unbounded, NoTick, Cluster<Replica>>>(replicas);

    let (proposers, acceptors, p_to_clients_new_leader_elected, p_to_replicas) =
        super::paxos::paxos_core(
            flow,
...
fn bench_client<'a, B: Address + Ord + std::fmt::Debug + Clone>(
