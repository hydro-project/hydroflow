...
pub fn collect_quorum<'a, L: Location<'a> + NoTick, K: Clone + Eq + Hash, V: Clone, E: Clone>(
    ...
    let (quorum_but_not_all_complete_cycle, quorum_but_not_all) = tick.cycle();
    ...
    let just_reached_quorum = p_p2b_quorum_reached.clone().anti_join(quorum_but_not_all); // Only tell the replicas about committed values once

    let received_from_all = count_per_key.filter_map(q!(move |(
        key,
        ((success, error), _values),
    )| if (success + error) >= max {
        Some(key)
    } else {
        None
    }));

    quorum_but_not_all_complete_cycle.complete_next_tick(
        p_p2b_quorum_reached
