...
pub fn paxos(
    ...
    let (p_is_leader_complete_cycle, p_is_leader) =
        flow.cycle::<_, _, Optional<bool, _, _, _>>(&proposers);

    let (p_to_proposers_i_am_leader_complete_cycle, p_to_proposers_i_am_leader) =
        flow.cycle::<_, _, Stream<Ballot, Unbounded, NoTick, Cluster<Proposer>>>(&proposers);
    let (a_to_proposers_p1b_complete_cycle, a_to_proposers_p1b) =
        flow.cycle::<_, _, Stream<(u32, P1b), _, _, _>>(&proposers);
    a_to_proposers_p1b
        .clone()
    ...
    let (a_to_proposers_p2b_complete_cycle, a_to_proposers_p2b) =
        flow.cycle::<_, _, Stream<(u32, P2b), _, _, _>>(&proposers);
...
fn acceptor<'a>(
    ...
    let a_log = a_p2as_to_place_in_log
        .union(a_new_checkpoint.as_stream())
        .persist()
        .fold(
...
fn p_p2a<'a>(
    ...
    p_ballot_num: Optional<'a, u32, Bounded, Tick, Cluster<Proposer>>,
    ...
    let (p_next_slot_complete_cycle, p_next_slot) =
        flow.cycle::<_, _, Optional<i32, _, _, _>>(proposers);
    ...
    let p_new_next_slot: Optional<i32, Bounded, Tick, Cluster<Proposer>> =
        p_new_next_slot_calculated
...
fn replica<'a>(
    ...
    let (r_highest_seq_complete_cycle, r_highest_seq) =
        flow.cycle::<_, _, Optional<'a, i32, _, _, _>>(replicas);
...
fn client<'a>(
    ...
    let (c_timers_complete_cycle, c_timers) =
        flow.cycle::<_, _, Stream<'a, (usize, SystemTime), _, _, _>>(clients);
    ...
    let c_latencies = c_timers
        ...
        .union(c_latency_reset.as_stream())
        .persist()
        .fold(
...
) -> Singleton<'a, Ballot, Bounded, Tick, Cluster<Proposer>> {
