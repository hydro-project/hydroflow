...
fn p_leader_expired<'a>(
    ...
    unsafe {
        // SAFETY: TODO
        p_latest_received_i_am_leader.latest_tick(proposer_tick)
    }
...
fn acceptor_p1<'a, L: Serialize + DeserializeOwned + Clone>(
    ...
    let a_max_ballot = p_to_acceptors_p1a
        ...
        .persist()
        .max()
        .unwrap_or(p_to_acceptors_p1a.location.singleton(q!(Ballot {
            num: 0,
            proposer_id: ClusterId::from_raw(0)
        })));
...
fn sequence_payload<'a, P: PaxosPayload, R>(
    ...
    let p_to_replicas = join_responses(proposer_tick, quorums, payloads_to_send).all_ticks();
...
fn index_payloads<'a, P: PaxosPayload>(
    ...
    let p_indexed_payloads = unsafe {
        // SAFETY: TODO
        c_to_proposers.tick_batch(proposer_tick)
    }
    .continue_if(p_is_leader)
    .enumerate()
...
fn acceptor_p2<'a, P: PaxosPayload, R>(
    ...
    let a_checkpoint_largest_seqs = unsafe {
        // SAFETY: TODO
        r_to_acceptors_checkpoint.tick_prefix(acceptor_tick)
    }
...
#[expect(clippy::type_complexity, reason = "internal paxos code // TODO")]
pub fn collect_quorum<'a, L: Location<'a> + NoTick, K: Clone + Eq + Hash, V: Clone, E: Clone>(
    tick: &Tick<L>,
    responses: Stream<(K, Result<V, E>), Tick<L>, Bounded, NoOrder>,
    min: usize,
    max: usize,
) -> (
    Stream<(K, Vec<V>), Tick<L>, Bounded, NoOrder>,
    Stream<(K, E), Tick<L>, Bounded, NoOrder>,
) {
    let (not_all_complete_cycle, not_all) = tick.cycle::<Stream<_, _, _, NoOrder>>();

    let current_responses = responses.clone().union(not_all);
    ...
    let reached_min_count =
        count_per_key
            .clone()
            .filter_map(q!(
                move |(key, ((success, _error), values))| if success >= min {
                    Some((key, values))
                } else {
                    None
                }
    ...
    let just_reached_quorum = if max == min {
        not_all_complete_cycle.complete_next_tick(
            current_responses.anti_join(reached_min_count.clone().map(q!(|(key, _values)| key))),
        );

        reached_min_count
    } else {
        let (min_but_not_max_complete_cycle, min_but_not_max) = tick.cycle();
        let just_reached_quorum = reached_min_count.clone().anti_join(min_but_not_max); // Only tell the replicas about committed values once

        let received_from_all =
            count_per_key.filter_map(q!(move |(key, ((success, error), _values))| if (success
                + error)
                >= max
            {
                Some(key)
        ...
        min_but_not_max_complete_cycle.complete_next_tick(
            reached_min_count
                .map(q!(|(key, _values)| key))
                .filter_not_in(received_from_all.clone()),
        );

        not_all_complete_cycle
            .complete_next_tick(current_responses.anti_join(received_from_all.clone()));
...
type JoinResponses<K, M, V, L> = Stream<(K, (M, V)), Tick<L>, Bounded, NoOrder>;
...
///
/// Only one response element should be produced with a given key, same for the metadata stream.
fn join_responses<'a, K: Clone + Eq + Hash, M: Clone, V: Clone, L: Location<'a> + NoTick>(
    tick: &Tick<L>,
    responses: Stream<(K, V), Tick<L>, Bounded, NoOrder>,
