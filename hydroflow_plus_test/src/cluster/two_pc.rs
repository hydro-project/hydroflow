...
use super::paxos::collect_quorum;
...
pub fn two_pc<'a>(
    ...
    let (c_all_commit, c_participant_voted_abort) = collect_quorum(
        &coordinator_tick,
        unsafe {
            // SAFETY: okay to batch because we immediately ignore batches on quorum
            c_received_reply
                .map(q!(|(id, (t, reply))| (
                    t,
                    if reply == "commit" { Ok(()) } else { Err(id) }
                )))
                .tick_batch(&coordinator_tick)
        },
    ...
    let p_receive_commit = c_all_commit
        .map(q!(|(k, _)| k))
        .all_ticks()
        .broadcast_bincode(&participants);
