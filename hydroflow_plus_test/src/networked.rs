...
use hydroflow_plus::node::{CLIRuntime, CLIRuntimeNodeBuilder, HfNetworkedDeploy, HfNode};
...
where
    D::Node: HfNode<'a, Port = String>,
{
    let node_zero = graph.node(node_builder);
    let node_one = graph.node(node_builder);

    let source_zero = graph.source_port(&node_zero, &"node_zero_input".to_string());
