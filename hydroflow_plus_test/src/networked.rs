...
use hydroflow_plus::node::{CLIRuntime, CLIRuntimeNodeBuilder, HfNetworkedDeploy};
...
pub fn networked_basic<'a, D: HfNetworkedDeploy<'a>>(
    ...
    let (source_zero_port, source_zero) = graph.source_external(&node_zero);
...
pub fn networked_basic_runtime<'a>(
    graph: &'a HfBuilder<'a, CLIRuntime<'a>>,
    cli: RuntimeData<&'a HydroCLI>,
    node_id: RuntimeData<usize>,
) -> impl Quoted<'a, Hydroflow<'a>> {
    let _ = networked_basic::<CLIRuntime>(graph, &mut CLIRuntimeNodeBuilder::new(cli));
    graph.build(node_id)
}
...
mod tests {
    ...
    async fn networked_basic() {
        ...
        let (source_zero_port, _, node_one) = super::networked_basic::<CLIDeploy>(
            &builder,
